...
def get_fixed_mod_raw(aa, data, params_dict, choices=None):
    dist_aa = []
    for ms, v in data.ms_stats().items():
        if choices is None or ms in choices:
            dist_aa.append([v[0], data.peptides(ms).apply(lambda x: x.count(aa)).sum()])
...
def determine_fixed_mods(aastat_result, aastat_df, locmod_df, data, params_dict):
    ...
    if reference == utils.mass_format(0):
        ...
        fix_mod_dict = determine_fixed_mods_zero(aastat_result, data, params_dict)
    else:
        ...
        loc = determine_fixed_mods_nonzero(reference, locmod_df, data)
        if loc:
...
def recalculate_counts(aa, ms_label, mods_and_counts, data):
    mods_and_counts[aa].pop(ms_label, None)
    for i, row in data.mass_shift(ms_label).iterrows():
        seq = row['top isoform'].split('.')[1]
        if row['top_terms'] is not None and ms_label in row['top_terms']:
            if aa == 'N-term' and seq[1] == '[':
                utils.internal('Reducing count of %s for %s (%s)', seq[0], seq, aa)
                if mods_and_counts[seq[0]].get(ms_label, 0) > 0:
                    mods_and_counts[seq[0]][ms_label] -= 1
            elif aa == 'C-term' and seq[-1] == ']':
                res = seq.split('[')[0][-1]
                utils.internal('Reducing count of %s for %s (%s)', res, seq, aa)
                if mods_and_counts[res].get(ms_label, 0) > 0:
                    mods_and_counts[res][ms_label] -= 1
            elif seq[:2] == aa + '[':
                utils.internal('Reducing count of N-term for %s', seq)
                if mods_and_counts['N-term'].get(ms_label, 0) > 0:
                    mods_and_counts['N-term'][ms_label] -= 1
            elif seq[-1] == ']' and seq.split('[')[0][-1] == aa:
                utils.internal('Reducing count of C-term for %s', seq)
                if mods_and_counts['C-term'].get(ms_label, 0) > 0:
                    mods_and_counts['C-term'][ms_label] -= 1


def recalculate_with_isotopes(aa, ms, isotope_rec, mods_and_counts, data, locmod_df):
    logger.debug('Recalculating counts for %s @ %s', aa, ms)
    recalculate_counts(aa, ms, mods_and_counts, data)
    i = 0
    while i < isotope_rec:
        label = utils.get_isotope_shift(ms, locmod_df)
        if label:
            logger.debug('Recalculating %s counts for isotope shift %s', aa, label)
            recalculate_counts(aa, label, mods_and_counts, data)
            i += 1
        else:
            break
        ...
        for i, row in df.iterrows():
            if row['top_terms'] is not None and ms in row['top_terms']:
                ...
                if same_residue(row['top isoform']):  # localization and enabled variable modification on the same residue
                    ...
                    sum_ms = utils.find_mass_shift(vm + shift, data, params_dict['prec_acc'])
                    if sum_ms:
...
def determine_var_mods(aastat_result, aastat_df, locmod_df, data, params_dict, recommended_fix_mods=None):
    if locmod_df is None:
    ...
    if recommended_fix_mods:
        ...
        for aa, shift in recommended_fix_mods.items():
            recalculate_with_isotopes(aa, shift, isotope_rec, mods_and_counts, data, locmod_df)
    ...
    if params_dict['var_mod']:
        ...
        recalculate_varmods(data, mods_and_counts, params_dict)
    ...
    for i in range(params_dict['variable_mods']):
        ...
        if mods_and_counts:
            ...
            if aa_shifts:
                ...
                recalculate_with_isotopes(top_aa, top_shift, isotope_rec, mods_and_counts, data, locmod_df)
                if not multiple:
